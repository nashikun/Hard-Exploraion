...
def init_parser():
    ...
    parser.add_argument("--seed", type=int, required=False, default=1, help="The random state seed. enter 0 for random")
    parser.add_argument("--batch_size", type=int, required=False, default=64, help="The batch size")
    ...
    parser.add_argument("--learning_rate", type=float, required=False, default=1e-3, help="The learning rate")
...
if __name__ == "__main__":
    ...
    algo = SoftActorCritic(env=config["env"], beta=config["beta"], is_frame=False, memory_size=1000,learning_rate=config["learning_rate"], seed=config["seed"])
    s = time.time()
